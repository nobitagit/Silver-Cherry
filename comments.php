...
<?php if ( have_comments() ) : ?>
	<section id="comments">
		<h3><?php comments_number(__('No comments until now.', 'reverie'), __('One comment.', 'reverie'), __('% comments', 'reverie') ); ?></h3>
    ...
    'reverse_top_level' => null )); ?>
		...
		<footer>
			<nav id="comments-nav">
				<div class="comments-previous left"><?php previous_comments_link( __( '<span class="white nice button radius fadeIn">Older comments</span>', 'reverie' ) ); ?></div>
				<div class="comments-next right"><?php next_comments_link( __( '<span class="white nice button radius fadeIn">Newer comments</span>', 'reverie' ) ); ?></div>
			</nav>
		</footer>
	</section>
...
<section id="respond">
 ...
 <?php printf(__('Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'reverie'), get_option('siteurl'), $user_identity); ?><br />
		<?php global $current_user; get_currentuserinfo();
       	 echo get_avatar( $current_user->ID, 65 ); ?>
         
          <br />
 			<a href="<?php echo wp_logout_url(get_permalink()); ?>" class="grey radius label fadeIn" title="<?php __('Log out of this account', 'reverie');  ?>"><?php _e('Log out &raquo;', 'reverie'); ?></a>
         </p>
		<?php else : ?>
		<p>
		...
		<?php endif; ?>
		<p>
			<label for="comment"><?php _e('Your message', 'reverie'); ?></label>
			<textarea name="comment" id="comment" tabindex="4"></textarea>
		</p>
		
		<p><input name="submit" class="cherry radius small button fadeIn" type="submit" id="submit" tabindex="5" value="<?php _e('Submit', 'reverie'); ?>"></p>
